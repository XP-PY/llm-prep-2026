...
**Group Relative Policy Optimization (GRPO)** is a reinforcement learning method for large language models that was introduced in the **DeepSeekMath** work. It is designed to improve reasoning ability while using **less memory than PPO**, mainly by **removing the separate critic/value model** and estimating “advantage” from a **group of sampled answers for the same prompt**. ([arXiv][1])
...
* **Critic-free**: it does not need a separate value network like PPO does. ([arXiv][1])
...
4. updating the model using those relative scores. ([arXiv][1])
...
because you can score each output automatically. ([arXiv][1])
...
6. Add a trust-region style constraint, usually via **clipping** and/or **KL control**, so updates do not become too aggressive. GRPO is described as a PPO variant, and TRL’s GRPO trainer also exposes KL-related controls in practice. ([arXiv][1])
...
So GRPO normalizes reward **within the sampled group for that prompt**. Follow-up theory and analyses describe GRPO in terms of exactly this kind of within-group normalization. ([arXiv][2])
...
That is the main memory/computation-saving idea of GRPO. ([arXiv][1])
...
\log \pi_\theta(y_i|x) - \log \pi_{\theta_{\text{old}}}(y_i|x)
]
...
GRPO is explicitly introduced as a PPO variant, and current TRL documentation presents it as a practical trainer for this style of RL post-training. ([arXiv][1])
...
* binary,
* sparse,
* or only meaningful when comparing several candidate outputs. Analyses of GRPO’s behavior emphasize this group-normalized contrastive structure. ([OpenReview][3])
...
* and it was used in the DeepSeekMath line and later reasoning-focused training efforts. ([arXiv][1])
...
TRL’s GRPO trainer also supports **multiple reward functions** and combines them in practice. ([Hugging Face][4])
...
This reduces memory usage and implementation complexity compared with PPO-style setups. ([arXiv][1])

### 11.2 Good for verifiable tasks

When you can automatically score outputs, GRPO is very natural. ([arXiv][1])

### 11.3 Relative scoring can stabilize learning

Even if raw rewards are rough, group normalization can still provide a useful “better/worse than peers” signal. Recent analyses formalize this relative-group structure. ([OpenReview][5])

### 11.4 Strong practical momentum

GRPO has become a very visible method in reasoning-focused open post-training pipelines, and recent theory papers explicitly describe it as a widely adopted default in this area. ([OpenReview][5])
...
If every sampled answer is wrong, group-relative normalization may provide a weak or misleading signal because the “best” answer is only best **relative to other bad answers**. This issue is discussed in recent follow-up work. ([OpenReview][6])
...
* or extra shaping rewards are added. Recent theory papers also study how group size affects GRPO’s behavior. ([arXiv][7])
